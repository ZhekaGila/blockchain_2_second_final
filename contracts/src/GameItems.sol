...
contract GameItems is ERC1155, Ownable {
    ...
    constructor(address initialOwner) ERC1155("https://gamefi.example/api/item/{id}.json") Ownable(initialOwner) {}

    function mint(address to, uint256 id, uint256 amount) external {
        require(
            msg.sender == owner() || msg.sender == craftingContract || msg.sender == itemShop, "Not allowed to mint"
        );
    ...
    function burnBatch(address from, uint256[] calldata ids, uint256[] calldata amounts) external {
        require(from == msg.sender || isApprovedForAll(from, msg.sender), "NoApproved");
